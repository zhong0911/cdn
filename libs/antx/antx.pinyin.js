 ...
 * 获取汉字的简码拼音
 */
Pinyin.getJianpin = function(str){
    str = str.toUpperCase();
    var len = str.length;
    var c, result="", t;
    for(i=0; i<len; i++){
...
Pinyin.GetHP = function(str){
    ...
    if (strlh == 2) {
        result = Pinyin.getQuanpin(regStr);
    } else {
        for(var i = 0;i < strlh; i++){
            if(i!=0){
                result += Pinyin.getJianpin(regStr.charAt(i).toString()) ;
            } else {
                result += Pinyin.getQuanpin(regStr.charAt(i).toString());
            }
        }
    }
    return result;
